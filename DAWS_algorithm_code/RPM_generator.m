...
for data_idx = 1:data_num
    ...
    elseif load_beat_signal == 1 && training_or_testing == 0 && breathing_or_heartbeat == 1
        ...
        load('./data/data_beat_train/heartbeat/walk_zhao.mat');%add by zyy
    ...
    radar_ADC_samples = 64;  % our adc samples
    ...
    if breathing_or_heartbeat == 0
        data_ref(1+window_num*(data_idx-1):window_num+window_num*(data_idx-1),1) = frequency_breathing * 60;
    elseif breathing_or_heartbeat == 1
        data_ref(1+window_num*(data_idx-1):window_num+window_num*(data_idx-1),2) = frequency_heartbeat * 60;
    end
